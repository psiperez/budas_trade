...
if ($contagem==0) {echo'<p align="center"><a href="../login2a_prof.php">Voce não tem acesso a esse paciente</a></p>';}
else {
    ...
    $registros = $conexao ->prepare ("SELECT atendimentos_diario.ID_ATENDIMENTOS,atendimentos_diario.DATA_HORA,atendimentos_diario.TEXTO,atendimentos_diario.EMAIL from atendimentos_diario inner join cadastro_usuario ON atendimentos_diario.EMAIL LIKE cadastro_usuario.EMAIL_USUARIO4 AND atendimentos_diario.CPF like :cpf ORDER BY atendimentos_diario.ID_ATENDIMENTOS ASC");
    $registros ->bindValue (':cpf',$cpf_usuario3_consulta);
    $registros ->execute();

    echo '<div style="position:absolute; left:0px; top:400px">';
    echo "<table align = 'center' border = '3' width='800px'>";
    ...
    // header("location:index_prof.php"); // Removido pois causa erro de headers already sent e impede visualização
