...
def calculate_backtest():
    ...
    # Period: 2025-03-14 to 2026-03-16
    ...
    contract_size = 100000 # Standard lot for EURUSD
    ...
    for i in range(2, len(df)):
        ...
        for p in positions[:]:
            ...
            if progress >= 0.25 and not p['p25']:
                ...
                history.append((close - p['entry_price']) * p['direction'] * close_lot * contract_size)
    ...
    plt.plot(balance_evolution, label='Saldo da Conta (Equity)', color='blue', linewidth=1.5)
    plt.axhline(y=100000, color='red', linestyle='--', alpha=0.5, label='Saldo Inicial')

    plt.title('Evolução do Saldo por Negociação - EURUSD H1 (Envelope Strategy)')
    plt.xlabel('Número da Negociação (incluindo parciais)')
    plt.ylabel('Capital (USD)')
    plt.legend()
    plt.grid(True, which='both', linestyle='--', alpha=0.5)

    plt.savefig('equity_curve_eurusd.png')
    print("\nGráfico salvo como 'equity_curve_eurusd.png'")
