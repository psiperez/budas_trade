//+------------------------------------------------------------------+
//| ENV_FIBO3 - Fibonacci Trend Envelope Version 1.00                |
//+------------------------------------------------------------------+
#property strict
...
void OnTick()
{
   ...
   if(range > 0)
   {
      ...
      // 3) Regra operacional: apenas 1 ordem pendente por vez (tipo stop)
      if(CountOrders() == 0 && CountPositions() == 0)
      {
         // Compra no cruzamento de 23.6% (Buy Stop se preço abaixo)
         if(ask < fibo236)
            PlacePendingStop(POSITION_TYPE_BUY, fibo236, smax, smin, "Fibo 23.6 Buy Stop");

         // Venda no cruzamento de 61.8% (Sell Stop se preço acima)
         else if(bid > fibo618)
            PlacePendingStop(POSITION_TYPE_SELL, fibo618, smin, smax, "Fibo 61.8 Sell Stop");
      }
   }
}
